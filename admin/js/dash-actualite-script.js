...
/*
  ✅ apiPostForm — used for CREATE (POST with image)
  Sends multipart/form-data so Cloudinary upload works.
...
/*
  ✅ apiPutForm — used for UPDATE (PUT with optional new image)
*/
...
/* =============================================
   ✅ LOAD — GET /api/actualite
   Your backend returns array sorted newest first.
   Fields: _id, title_en, title_fr, category,
           status, icon, text_en, text_fr,
           image_url, image_public_id, createdAt
============================================= */
function loadItems() {
...
function renderGrid() {
  ...
  if (page.length === 0) {
    grid.innerHTML =
      ...
      '<h3>' + (lang==='fr' ? 'Aucune actualité trouvée' : 'No actualités found') + '</h3>' +
...
var iconEl = document.getElementById('modal-form-icon');
if (iconEl) {
  iconEl.className = 'modal-header-icon edit';
  iconEl.innerHTML = '<i class="ph ph-pencil-simple"></i>';
}

var titleEl = document.getElementById('modal-form-title');
if (titleEl) {
  titleEl.textContent = lang === 'fr' ? 'Modifier l\'Actualité' : 'Edit Actualité';
}

var subEl = document.getElementById('modal-form-subtitle');
if (subEl) {
  subEl.textContent = lang === 'fr' ? (item.title_fr || '') : (item.title_en || '');
}

var btnEl = document.getElementById('submit-btn-label');
if (btnEl) {
  btnEl.textContent = lang === 'fr' ? 'Mettre à Jour' : 'Update Actualité';
}
  openModal('modal-form');
}
...
document.getElementById('confirm-delete-btn').addEventListener('click', function() {
  ...
  function afterDel() {
    STATE.items = STATE.items.filter(function(i) { return uid(i) !== id; });
    applyFilters();
    closeModal('modal-delete');
    showToast('success',
      lang==='fr' ? 'Actualité supprimée' : 'Actualité deleted',
      '"' + STATE.deletingName + '" ' + (lang==='fr' ? 'supprimée.' : 'removed.')
    );
...
/* =============================================
   ✅ FORM SUBMIT
   ADD:  POST /api/actualite      (multipart/form-data)
   EDIT: PUT  /api/actualite/:id  (multipart/form-data)

   Sends FormData so image uploads to Cloudinary.
...
document.getElementById('submit-form').addEventListener('click', function() {
  ...
  var request = isEdit
    ? apiPutForm('/actualite/' + STATE.editingId, formData)
    : apiPostForm('/actualite', formData);
  ...
  function afterSave(data) {
    if (!data) {
      showToast('error', 'Error', 'Server returned no data.');
      btn.disabled  = false;
      btn.innerHTML = '<i class="ph ph-floppy-disk"></i> <span>' + (lang==='fr' ? 'Enregistrer' : 'Save Actualité') + '</span>';
      return;
    }
    ...
    if (isEdit) {
      var idx = STATE.items.findIndex(function(i) { return uid(i) === STATE.editingId; });
    ...
    showToast('success',
      isEdit ? (lang==='fr' ? 'Mise à jour' : 'Updated') : (lang==='fr' ? 'Ajoutée' : 'Added'),
      '"' + title_en + '" ' + (lang==='fr' ? 'enregistrée.' : 'saved.')
    );
    btn.disabled  = false;
    btn.innerHTML = '<i class="ph ph-floppy-disk"></i> <span>' + (lang==='fr' ? 'Enregistrer' : 'Save Actualité') + '</span>';
  }
...
/*
  ═══════════════════════════════════════════════════
  BACKEND NEEDED — dash-actualite-script.js
  ═══════════════════════════════════════════════════

  Build these routes + controller (same pattern as Services):

  Model fields:
    title_en, title_fr   (String, required)
    text_en,  text_fr    (String, required)
    category             (String, enum: mining/environment/regulation/innovation/community/event)
    status               (String, enum: published/draft, default: published)
    icon                 (String, default: ph-megaphone)
    image_url            (String)
    image_public_id      (String)   ← for Cloudinary deletion

  Routes (all protected):
    GET    /api/actualite      → getActualites
    GET    /api/actualite/:id  → getActualiteById
    POST   /api/actualite      → createActualite  (upload.single('image'))
    PUT    /api/actualite/:id  → updateActualite  (upload.single('image'))
    DELETE /api/actualite/:id  → deleteActualite  (also destroys Cloudinary image)
